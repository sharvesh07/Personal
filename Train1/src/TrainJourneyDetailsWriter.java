...
public class TrainJourneyDetailsWriter {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < numberOfTrains; i++) {
            ...
            journeys.add(new TrainJourneyDetails(trainNumber, stations));
        }
    ...
    private static void writeJourneyDetails(List<TrainJourneyDetails> journeys) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(journeyFilePath))) {
            for (TrainJourneyDetails journey : journeys) {
                writer.write(journey.getTrainNumber() + ";" + String.join(",", journey.getStations()));
                writer.newLine();
            }
        } catch (IOException e) {
...
class TrainJourneyDetails {
    ...
    public TrainJourneyDetails(int trainNumber, String[] stations) {
